...
void Calendar_Populate(Display_Type *FanledDisplay, sytemdate_t *sysDate)
{
	...
	for(i = 15; i < 31; i++) FanledDisplay->dis[sysDate->ss*4][i] = CALENDAR_SECOND_STICK_COLOR;
	for(i = 0; i <= sysDate->ss*4; i++) FanledDisplay->dis[i][3] = CALENDAR_SECOND_STICK_COLOR;
	//Draw Minute stick and circle
	for(i = 18; i < 31; i++) FanledDisplay->dis[sysDate->mm*4][i] = CALENDAR_MINUTE_STICK_COLOR;
	for(i = 0; i <= sysDate->mm*4; i++) FanledDisplay->dis[i][4] = CALENDAR_MINUTE_STICK_COLOR;
	//Draw hour stick and circle
	for(i = 20; i < 31; i++) FanledDisplay->dis[(sysDate->hh%12)*20][i] = CALENDAR_HOUR_STICK_COLOR;
	for(i = 0; i <= (sysDate->hh%12)*20; i++) FanledDisplay->dis[i][5] = CALENDAR_HOUR_STICK_COLOR;
}
